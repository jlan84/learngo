...
func main() {
	...
	fmt.Println(s)
	if len(args) != 1 {
		fmt.Println("Please enter one name to display a mood")
		return
	}
	feelings := [...]string{" feels happy", " feels sad",
		" feels positive", " feels negative",
	}
	feelings[0] = " feels sour"
	i := rand.Intn(len(feelings))
	fmt.Printf("%s%s\n", args[0], feelings[i])
}
